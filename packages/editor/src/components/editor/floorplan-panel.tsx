...
function formatMeasurement(value: number, unit: LinearUnit, metersPerUnit: number | null = null) {
...
function convertReferenceLengthToMeters(value: number, unit: ReferenceScaleUnit) {
  switch (unit) {
    case 'centimeters':
      return linearUnitToMeters(value, 'centimeter')
    case 'millimeters':
      return linearUnitToMeters(value, 'millimeter')
    case 'feet':
...
function getReferenceScaleUnitForLinearUnit(unit: LinearUnit): ReferenceScaleUnit {
  if (unit === 'imperial') return 'feet'
  if (unit === 'centimeter') return 'centimeters'
  if (unit === 'millimeter') return 'millimeters'
  return 'meters'
}

function metersToReferenceScaleValue(meters: number, unit: ReferenceScaleUnit) {
  switch (unit) {
    case 'centimeters':
      return meters / linearUnitToMeters(1, 'centimeter')
    case 'millimeters':
      return meters / linearUnitToMeters(1, 'millimeter')
    case 'feet':
      return meters / linearUnitToMeters(1, 'imperial')
    case 'inches':
...
function formatArea(areaSqM: number, unit: LinearUnit, metersPerUnit: number | null = null) {
  const scaledAreaSqM =
  ...
  if (unit === 'centimeter') {
    const areaSqCm = scaledAreaSqM * 10_000
    return (
      <>
        {Math.round(areaSqCm).toLocaleString()}
        <tspan dx="0.12em">cm</tspan>
        <tspan baselineShift="super" fontSize="0.75em">
          2
        </tspan>
      </>
  ...
  const areaSqMm = scaledAreaSqM * 1_000_000
  return (
    <>
      {Math.round(areaSqMm).toLocaleString()}
      <tspan dx="0.12em">mm</tspan>
      <tspan baselineShift="super" fontSize="0.75em">
        2
      </tspan>
    </>
...
export function FloorplanPanel({
  ...
  const [referenceScaleUnit, setReferenceScaleUnit] = useState<ReferenceScaleUnit>(
    getReferenceScaleUnitForLinearUnit(unit),
  )
  ...
  useEffect(() => {
    setReferenceScaleUnit(getReferenceScaleUnitForLinearUnit(unit))
  }, [unit])
  ...
  const handleBackgroundClick = useCallback(
    (event: ReactMouseEvent<SVGSVGElement>) => {
      ...
      if (referenceScaleDraft) {
        ...
        // confirming without editing is a no-op instead of a surprise rescale.
        const defaultReferenceUnit = getReferenceScaleUnitForLinearUnit(unit)
        setReferenceScaleValue(
          formatNumber(metersToReferenceScaleValue(measuredLengthUnits, defaultReferenceUnit), 2),
        )
        setReferenceScaleUnit(defaultReferenceUnit)
        setReferenceScaleDraft(null)
        setCursorPoint(null)
        return
      }
