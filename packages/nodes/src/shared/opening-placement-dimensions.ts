...
export function buildOpeningPlacementDimensions(
  ...
  const formatLength = (value: number) => {
    if (ctx.viewState?.unit === 'imperial') {
      const feetValue = Math.abs(value) / 0.3048
      ...
      const sign = value < 0 && (feet !== 0 || inches !== 0) ? '-' : ''
      return `${sign}${feet}'${inches}"`
    }
    if (ctx.viewState?.unit === 'centimeter') {
      return `${Math.round(value * 1000) / 10}cm`
    }
  ...
  for (const gap of guides.gaps) {
    ...
    out.push({
      ...
      text: formatLength(gap.distance),
      stroke: '#f97316',
    })
  }
  ...
  if (guides.equalSpacing) {
    ...
    const text = formatLength(guides.equalSpacing.gap)
