...
function normalizeViewerUnit(value: unknown): ViewerUnit {
  const unit = pickString<ViewerUnit>(value, UNITS, 'millimeter')
  return unit === 'metric' ? 'millimeter' : unit
}
...
const useViewer = create<ViewerState>()(
  persist(
    (set) => ({
      ...
      unit: 'millimeter',
      setUnit: (unit) => set({ unit: unit === 'metric' ? 'millimeter' : unit }),
